...
-(id) init
{
	...
	if( (self=[super init])) {
		...
		// Caching your physics shapes
		[[GB2ShapeCache sharedShapeCache] addShapesWithFile:@"shapedefs.plist"];
		
		// Needed for pvr.ccz file format
		[CCTexture2D PVRImagesHavePremultipliedAlpha:YES];
		
		// Setting up default alpha pixel format (change the last bit "RGBA8888" if you are using a different format)
		[CCTexture2D setDefaultAlphaPixelFormat:kCCTexture2DPixelFormat_RGBA8888];
		
		// Creating and adding batch node information (SpriteSheet)
...
-(void) addNewSpriteWithCoords:(CGPoint)p
{
	...
	CCLOG(@"Add sprite %@ %0.2f x %02.f", name, p.x, p.y);   
	
	CCSprite *sprite = [CCSprite spriteWithSpriteFrameName:[NSString stringWithFormat:@"%@.png", name]];
